...
class TestLambdaHandler(unittest.TestCase):
    ...
    def test_config_object(self):
        ...
        assert_that(
            self.maxmind_downloader.download_location, equal_to(self.download_location)
        )
    ...
    def test_maxmind_fetch_file(self, m):
        ...
        m.get(
            ...
            "edition_id=test&suffix=tar.gz.md5&license_key=testlicense",
            text=self.tar_file_md5,
        )
        m.get(
            "https://download.maxmind.com/app/geoip_download?"
            "edition_id=test&suffix=tar.gz&license_key=testlicense",
            body=test_file,
        )
    ...
    def test_maxmind_lambda(self, m):
        ...
        m.get(
            "https://download.maxmind.com/app/geoip_download?"
            "edition_id=test1&suffix=zip.md5&license_key=testlicense",
            text=self.zip_file_md5,
        )
        m.get(
            "https://download.maxmind.com/app/geoip_download?"
            "edition_id=test1&suffix=zip&license_key=testlicense",
            body=test_zip_file,
        )
        ...
        assert_that(
            response["Contents"][0]["Key"], equal_to("test1/20190101-test1.mmdb")
        )
        assert_that(
            response["Contents"][1]["Key"],
            equal_to("test1/20190102-test-Locations-en.csv"),
        )
